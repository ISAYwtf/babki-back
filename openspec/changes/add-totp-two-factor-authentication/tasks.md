...
- [ ] 10.4 Perform the phased release check: deploy with enrollment disabled, verify every instance enforces the new login path and zero-version JWT compatibility, then enable enrollment and document that rollback to password-only binaries is prohibited once any credential is enabled.
